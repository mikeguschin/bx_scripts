...
<script type="text/javascript">
	...
	window.GLOBAL_arMapObjects[objName].geoObjects.events.add('click', function(ob) {
		var target = ob.get('target'),
			title = target.properties.get('hintContent');
		for(var id in arStore){
			if(arStore.hasOwnProperty(id) && arStore[id].TITLE === title){
				setChangeStore(id);
				break;
			}
		}
